...
public class EngiesOwnEngieBundleItem extends Item {
	public EngiesOwnEngieBundleItem() {
	...
	public void appendHoverText(ItemStack itemstack, Level level, List<Component> list, TooltipFlag flag) {
		...
		list.add(Component.translatable("item.allaboutengie.engies_own_engie_bundle.description_0"));
	}
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openScreen(serverPlayer, new MenuProvider() {
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new EngiesOwnEngieBundleUIMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
	...
	public ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag compound) {
		return new EngiesOwnEngieBundleInventoryCapability();
	}
