...
public class OutragedEngieBundleInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == AllaboutengieModItems.OUTRAGED_ENGIE_BUNDLE.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(80) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != AllaboutengieModItems.OUTRAGED_ENGIE_BUNDLE.get();
			}
