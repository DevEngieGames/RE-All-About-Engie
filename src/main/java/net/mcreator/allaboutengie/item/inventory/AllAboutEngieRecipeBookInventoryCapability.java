...
public class AllAboutEngieRecipeBookInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == AllaboutengieModItems.ALL_ABOUT_ENGIE_RECIPE_BOOK.get()) {
			if (Minecraft.getInstance().screen instanceof CustomRecipeBookScreen) {
