...
public class CreativeEngieBundleItem extends Item {
	...
	public void appendHoverText(ItemStack itemstack, Level level, List<Component> list, TooltipFlag flag) {
		...
		list.add(Component.translatable("item.allaboutengie.creative_engie_bundle.description_0"));
		list.add(Component.translatable("item.allaboutengie.creative_engie_bundle.description_1"));
	}
		...
		if (entity instanceof ServerPlayer serverPlayer) {
			NetworkHooks.openScreen(serverPlayer, new MenuProvider() {
				...
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					...
					return new CreativeEngieBundleUIMenu(id, inventory, packetBuffer);
				}
			}, buf -> {
