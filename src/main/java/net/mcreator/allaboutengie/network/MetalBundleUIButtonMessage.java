...
public class MetalBundleUIButtonMessage {
	...
	public static void handleButtonAction(Player entity, int buttonID, int x, int y, int z) {
		...
		HashMap guistate = MetalBundleUIMenu.guistate;
		// security measure to prevent arbitrary chunk generation
		if (!world.hasChunkAt(new BlockPos(x, y, z)))
			return;
		if (buttonID == 0) {

			MetalBundleNameChangeProcedure.execute(entity, guistate);
		}
	}

	@SubscribeEvent
	public static void registerMessage(FMLCommonSetupEvent event) {
		AllaboutengieMod.addNetworkMessage(MetalBundleUIButtonMessage.class, MetalBundleUIButtonMessage::buffer, MetalBundleUIButtonMessage::new, MetalBundleUIButtonMessage::handler);
	}
}
