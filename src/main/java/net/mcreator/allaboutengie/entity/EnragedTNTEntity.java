...
import net.mcreator.allaboutengie.procedures.TNTSpawnTNTDeathProcedure;
import net.mcreator.allaboutengie.procedures.MutationchanceProcedure;
...
public class EnragedTNTEntity extends Monster {
	public EnragedTNTEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AllaboutengieModEntities.ENRAGED_TNT.get(), world);
	}

	public EnragedTNTEntity(EntityType<EnragedTNTEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 15;
		setNoAi(false);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		MutationchanceProcedure.execute(world, this);
		return retval;
	}

	public static void init() {
		SpawnPlacements.register(AllaboutengieModEntities.ENRAGED_TNT.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
