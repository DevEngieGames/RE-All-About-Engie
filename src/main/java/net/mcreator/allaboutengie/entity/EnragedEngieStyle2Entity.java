...
public class EnragedEngieStyle2Entity extends Monster {
	public EnragedEngieStyle2Entity(PlayMessages.SpawnEntity packet, Level world) {
		this(AllaboutengieModEntities.ENRAGED_ENGIE_STYLE_2.get(), world);
	}

	public EnragedEngieStyle2Entity(EntityType<EnragedEngieStyle2Entity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 25;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(5, new NearestAttackableTargetGoal(this, Monster.class, true, false) {
			...
			public boolean canUse() {
				double x = EnragedEngieStyle2Entity.this.getX();
				double y = EnragedEngieStyle2Entity.this.getY();
				double z = EnragedEngieStyle2Entity.this.getZ();
				Entity entity = EnragedEngieStyle2Entity.this;
				Level world = EnragedEngieStyle2Entity.this.level();
	...
	public static void init() {
		SpawnPlacements.register(AllaboutengieModEntities.ENRAGED_ENGIE_STYLE_2.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
