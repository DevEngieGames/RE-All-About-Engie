...
public class EnragedEngieSharkoEntity extends Monster {
	public EnragedEngieSharkoEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AllaboutengieModEntities.ENRAGED_ENGIE_SHARKO.get(), world);
	}

	public EnragedEngieSharkoEntity(EntityType<EnragedEngieSharkoEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 25;
		setNoAi(false);
	...
	public static void init() {
		SpawnPlacements.register(AllaboutengieModEntities.ENRAGED_ENGIE_SHARKO.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
