...
public class ThrowbackEnragedEngieEntity extends Monster {
	public ThrowbackEnragedEngieEntity(EntityType<ThrowbackEnragedEngieEntity> type, Level world) {
		super(type, world);
		xpReward = 25;
		setNoAi(false);
	}
	...
	public static void init(RegisterSpawnPlacementsEvent event) {
		event.register(AllaboutengieModEntities.THROWBACK_ENRAGED_ENGIE.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
