...
import net.mcreator.allaboutengie.procedures.TheEndHostileEntityDiesProcedure;
...
public class TheEndHostileEntity extends Monster {
	...
	public TheEndHostileEntity(EntityType<TheEndHostileEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	public void die(DamageSource source) {
		super.die(source);
		TheEndHostileEntityDiesProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}
	...
	public static void init() {
		SpawnPlacements.register(AllaboutengieModEntities.THE_END_HOSTILE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
