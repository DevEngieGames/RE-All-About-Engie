...
public class OutragedEngieStyle2Entity extends Monster {
	public OutragedEngieStyle2Entity(PlayMessages.SpawnEntity packet, Level world) {
		this(AllaboutengieModEntities.OUTRAGED_ENGIE_STYLE_2.get(), world);
	}

	public OutragedEngieStyle2Entity(EntityType<OutragedEngieStyle2Entity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 35;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(5, new NearestAttackableTargetGoal(this, Monster.class, true, false) {
			...
			public boolean canUse() {
				double x = OutragedEngieStyle2Entity.this.getX();
				double y = OutragedEngieStyle2Entity.this.getY();
				double z = OutragedEngieStyle2Entity.this.getZ();
				Entity entity = OutragedEngieStyle2Entity.this;
				Level world = OutragedEngieStyle2Entity.this.level();
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(AllaboutengieModItems.OUTRAGED_ENGIE_ESSENCE.get()));
	}
	...
	public static void init() {
		SpawnPlacements.register(AllaboutengieModEntities.OUTRAGED_ENGIE_STYLE_2.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
