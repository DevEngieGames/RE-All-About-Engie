...
public class ThrowbackMadEngieEntity extends Monster {
	public ThrowbackMadEngieEntity(EntityType<ThrowbackMadEngieEntity> type, Level world) {
		super(type, world);
		xpReward = 10;
		setNoAi(false);
	}
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(5, new NearestAttackableTargetGoal(this, Monster.class, true, false) {
			...
			public boolean canUse() {
				double x = ThrowbackMadEngieEntity.this.getX();
				double y = ThrowbackMadEngieEntity.this.getY();
				double z = ThrowbackMadEngieEntity.this.getZ();
				Entity entity = ThrowbackMadEngieEntity.this;
				Level world = ThrowbackMadEngieEntity.this.level();
	...
	protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource source, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(serverLevel, new ItemStack(AllaboutengieModItems.THROWBACK_ENGIE_GEM.get()));
	}
	...
	public static void init(RegisterSpawnPlacementsEvent event) {
		event.register(AllaboutengieModEntities.THROWBACK_MAD_ENGIE.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
