...
public class EpicSharkoEntity extends PathfinderMob {
	...
	public EpicSharkoEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AllaboutengieModEntities.EPIC_SHARKO.get(), world);
	}

	public EpicSharkoEntity(EntityType<EpicSharkoEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	public static void init() {
		SpawnPlacements.register(AllaboutengieModEntities.EPIC_SHARKO.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
