...
public class AngryEngieSharkoEntity extends Monster {
	public AngryEngieSharkoEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AllaboutengieModEntities.ANGRY_ENGIE_SHARKO.get(), world);
	}

	public AngryEngieSharkoEntity(EntityType<AngryEngieSharkoEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 15;
		setNoAi(false);
	...
	public static void init() {
		SpawnPlacements.register(AllaboutengieModEntities.ANGRY_ENGIE_SHARKO.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
