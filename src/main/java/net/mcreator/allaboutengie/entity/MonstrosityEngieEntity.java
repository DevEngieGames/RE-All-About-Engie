...
import net.mcreator.allaboutengie.procedures.MonstrosityAndBloodySpawningConditionProcedure;
...
public class MonstrosityEngieEntity extends Monster {
	public MonstrosityEngieEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AllaboutengieModEntities.MONSTROSITY_ENGIE.get(), world);
	}
	...
	public static void init() {
		SpawnPlacements.register(AllaboutengieModEntities.MONSTROSITY_ENGIE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return MonstrosityAndBloodySpawningConditionProcedure.execute(world);
		});
	}
