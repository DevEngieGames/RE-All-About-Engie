...
public class AtlasSitRenderer extends MobRenderer<AtlasSitEntity, Modeltobysitnew<AtlasSitEntity>> {
	...
	@Override
	public ResourceLocation getTextureLocation(AtlasSitEntity entity) {
		return ResourceLocation.parse("allaboutengie:textures/entities/atlas.png");
	}

	private static final class AnimatedModel extends Modeltobysitnew<AtlasSitEntity> {
		private final ModelPart root;
		private final HierarchicalModel animator = new HierarchicalModel<AtlasSitEntity>() {
			@Override
			...
			@Override
			public void setupAnim(AtlasSitEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
