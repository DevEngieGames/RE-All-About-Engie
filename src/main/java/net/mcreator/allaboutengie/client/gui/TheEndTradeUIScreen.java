...
public class TheEndTradeUIScreen extends AbstractContainerScreen<TheEndTradeUIMenu> implements AllaboutengieModScreens.ScreenAccessor {
	...
	public TheEndTradeUIScreen(TheEndTradeUIMenu container, Inventory inventory, Component text) {
	...
	protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
		guiGraphics.drawString(this.font, Component.translatable("gui.allaboutengie.the_end_trade_ui.label_trade_ui_state_normal"), 5, -11, -1, false);
	}
	...
	public void init() {
		...
		button_trade = Button.builder(Component.translatable("gui.allaboutengie.the_end_trade_ui.button_trade"), e -> {
			...
			if (true) {
				PacketDistributor.sendToServer(new TheEndTradeUIButtonMessage(0, x, y, z));
		...
		button_swap = Button.builder(Component.translatable("gui.allaboutengie.the_end_trade_ui.button_swap"), e -> {
			int x = TheEndTradeUIScreen.this.x;
			int y = TheEndTradeUIScreen.this.y;
			if (true) {
				PacketDistributor.sendToServer(new TheEndTradeUIButtonMessage(1, x, y, z));
				TheEndTradeUIButtonMessage.handleButtonAction(entity, 1, x, y, z);
			}
		}).bounds(this.leftPos + 9, this.topPos + 6, 51, 20).build();
		this.addRenderableWidget(button_swap);
		scythetrade = Checkbox.builder(Component.translatable("gui.allaboutengie.the_end_trade_ui.scythetrade"), this.font).pos(this.leftPos + 71, this.topPos + 6).onValueChange((checkbox, value) -> {
			if (!menuStateUpdateActive)
				menu.sendMenuStateUpdate(entity, 1, "scythetrade", value, false);
		}).build();
		this.addRenderableWidget(scythetrade);
		bantrade = Checkbox.builder(Component.translatable("gui.allaboutengie.the_end_trade_ui.bantrade"), this.font).pos(this.leftPos + 98, this.topPos + 6).onValueChange((checkbox, value) -> {
			if (!menuStateUpdateActive)
