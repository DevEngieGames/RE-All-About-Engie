...
public class BigLegendaryTameRCProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == Items.COOKIE) {
			...
			AllaboutengieMod.queueServerWork(1, () -> {
				if ((findEntityInWorldRange(world, BigLegendarySharkoLayEntity.class, x, y, z, 1)) instanceof BigLegendarySharkoLayEntity) {
					if (!(entity.getDisplayName().getString()).equals("[Tamed] {Big} Legendary Sharko")) {
						(findEntityInWorldRange(world, BigLegendarySharkoLayEntity.class, x, y, z, 1)).setCustomName(Component.literal((entity.getDisplayName().getString())));
					}
					if ((findEntityInWorldRange(world, BigLegendarySharkoLayEntity.class, x, y, z, 1)) instanceof LivingEntity _entity)
