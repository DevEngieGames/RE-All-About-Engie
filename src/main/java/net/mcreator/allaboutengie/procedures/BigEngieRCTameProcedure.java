...
public class BigEngieRCTameProcedure {
		...
		if (!world.isClientSide()) {
			if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == AllaboutengieModItems.ENGIE_COOKIE.get()) {
				if (Math.random() >= 0.8) {
					...
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = AllaboutengieModEntities.BIG_ENGIE_SHARKO_TAMED.get().spawn(_level, BlockPos.containing(x, y, z), EntitySpawnReason.MOB_SUMMONED);
