	...
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == Items.COOKIE) {
			...
			AllaboutengieMod.queueServerWork(1, () -> {
				if ((findEntityInWorldRange(world, BigEngieSharkoRareLayEntity.class, x, y, z, 1)) instanceof BigEngieSharkoRareLayEntity) {
					if (!(entity.getDisplayName().getString()).equals("[Tamed] {Big} Rare Engie Sharko")) {
						(findEntityInWorldRange(world, BigEngieSharkoRareTamedEntity.class, x, y, z, 1)).setCustomName(Component.literal((entity.getDisplayName().getString())));
					}
