...
public class BigEngieRare2LayRCProcedure {
				...
				entity.discard();
			{
				...
				if (!_ent.level().isClientSide() && _ent.getServer() != null) {
					_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
							_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), "summon allaboutengie:big_tiger_sharko_tamed ~ ~ ~");
				}
			}
			AllaboutengieMod.queueServerWork(1, () -> {
				if ((findEntityInWorldRange(world, BigEngieSharkoRare2TamedEntity.class, x, y, z, 1)) instanceof BigEngieSharkoRare2TamedEntity) {
					if (!(entity.getDisplayName().getString()).equals("[Tamed] {Big} Tiger Sharko (Laying)")) {
						(findEntityInWorldRange(world, BigEngieSharkoRare2TamedEntity.class, x, y, z, 1)).setCustomName(Component.literal((entity.getDisplayName().getString())));
					}
					if ((findEntityInWorldRange(world, BigEngieSharkoRare2TamedEntity.class, x, y, z, 1)) instanceof LivingEntity _entity)
