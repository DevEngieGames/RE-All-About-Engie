...
public class TheEndChaosProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (!world.isClientSide()) {
			if (AllaboutengieModVariables.MapVariables.get(world).thestart == true && AllaboutengieModVariables.MapVariables.get(world).TheEndStart == true) {
				...
				if (entity.getPersistentData().getDouble("theendmainsongtimer") >= 140) {
					...
					AllaboutengieModVariables.MapVariables.get(world).theendmainsongstart = false;
					AllaboutengieModVariables.MapVariables.get(world).syncData(world);
					AllaboutengieMod.queueServerWork(1, () -> {
						if (AllaboutengieModVariables.MapVariables.get(world).theendmainsongstart == false) {
							AllaboutengieModVariables.MapVariables.get(world).theendmainsongstart = true;
							AllaboutengieModVariables.MapVariables.get(world).syncData(world);
							if (world instanceof Level _level) {
								if (!_level.isClientSide()) {
									_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("allaboutengie:doomsday_start")), SoundSource.MUSIC, (float) 1.5, 1);
								} else {
									_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("allaboutengie:doomsday_start")), SoundSource.MUSIC, (float) 1.5, 1, false);
								}
							}
						}
					});
					...
					if (entity.getPersistentData().getDouble("theendtimerhalf2") >= 720) {
						...
						if (entity.getData(AllaboutengieModVariables.PLAYER_VARIABLES).DoomsdayAlive == true) {
							if (!(entity instanceof ServerPlayer _plr25 && _plr25.level() instanceof ServerLevel
									&& _plr25.getAdvancements().getOrStartProgress(_plr25.server.getAdvancements().get(ResourceLocation.parse("allaboutengie:the_one"))).isDone())) {
								if (entity instanceof ServerPlayer _player) {
									AdvancementHolder _adv = _player.server.getAdvancements().get(ResourceLocation.parse("allaboutengie:the_one"));
									if (_adv != null) {
							...
							if (!(entity instanceof ServerPlayer _plr27 && _plr27.level() instanceof ServerLevel
									&& _plr27.getAdvancements().getOrStartProgress(_plr27.server.getAdvancements().get(ResourceLocation.parse("allaboutengie:conqueror"))).isDone())) {
								if (entity instanceof ServerPlayer _player) {
