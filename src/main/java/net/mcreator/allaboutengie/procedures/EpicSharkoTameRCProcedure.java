...
public class EpicSharkoTameRCProcedure {
				...
				entity.discard();
			{
				...
				if (!_ent.level().isClientSide() && _ent.getServer() != null) {
					_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
							_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), "summon allaboutengie:epic_sharko_sit ~ ~ ~");
				}
			}
			AllaboutengieMod.queueServerWork(1, () -> {
				if ((findEntityInWorldRange(world, EpicSharkoSitEntity.class, x, y, z, 1)) instanceof EpicSharkoSitEntity) {
					if (!(entity.getDisplayName().getString()).equals("Epic Sharko")) {
						(findEntityInWorldRange(world, EpicSharkoSitEntity.class, x, y, z, 1)).setCustomName(Component.literal((entity.getDisplayName().getString())));
					}
					if ((findEntityInWorldRange(world, EpicSharkoSitEntity.class, x, y, z, 1)) instanceof LivingEntity _entity)
